...
//   const plus = event.target.closest(".card-qty__plus");
//   const minus = event.target.closest(".card-qty__minus");

//   if (!plus && !minus) return;

//   const qtyBox = event.target.closest(".card-qty");
//   if (!qtyBox) return;

//   const variantId = qtyBox.dataset.variantId;
//   const number = qtyBox.querySelector(".card-qty__number");
//   let currentQty = parseInt(number.textContent, 10) || 0;
...
//     number.textContent = newQty;
//   }
// });
// טיפול בכפתורי + / - בכרטיס מוצר (ללא מעבר לדף מוצר)
// חשוב: משתמשים ב־event delegation כדי שיפעל על כל המוצרים
// וגם מונעים מעבר ללינק של הכרטיס (כי כל הכרטיס עטוף ב־<a> ב־Dawn)
function flyToCart(fromElement) {
  ...
  const rectTo = cart.getBoundingClientRect();
...
async function updateCartCount() {
  ...
  if (!bubble && cart.item_count > 0) {
    ...
    bubble.innerHTML = `
      ...
      <span class="visually-hidden">${cart.item_count} פריטים בסל</span>
    `;
  ...
  if (bubble) {
    ...
    if (hidden) hidden.textContent = `${cart.item_count} פריטים בסל`;

    if (cart.item_count === 0) {
      bubble.remove();
    } else {
      bubble.classList.add("cart-bump");
      setTimeout(() => bubble.classList.remove("cart-bump"), 300);
    }
  }
}
